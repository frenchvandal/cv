...
carries two `<?xml-stylesheet?>` instructions, both generated by
[scripts/sitemap-style.ts](scripts/sitemap-style.ts) and both invisible to
crawlers and to `parseSitemap`:

- `dist/sitemap.xsl` transforms the XML into a table of links, one row per
...
either path by hand with the flag Chrome will ship in 158:

```bash
open -a "Google Chrome" --args --disable-features=XSLT
```
